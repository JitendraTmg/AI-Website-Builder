  ...
  try {
    ...
    if (user.credits < 10) {
      return res.status(400).json({
        message: "Not enough credits to generate website. Please upgrade your plan."
      })
    }
    ...
    let raw = "";
    let parsed = null;
    // while creating website, if the response is not in correct format, try regenerating it 2 times before giving up and returning an error to the user
...
export const changeWebsite = async (req, res) => {
  try {
    ...
    if (user.credits < 5) {
      return res.status(400).json({
        message: "Not enough credits to modify website. Please upgrade your plan."
      })
    }
    const udpatePrompt = `
